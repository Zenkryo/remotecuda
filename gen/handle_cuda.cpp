...
int handle_cuLogsRegisterCallback(void *args0) {
#ifdef DEBUG
    std::cout << "Handle function cuLogsRegisterCallback called" << std::endl;
#endif
    int rtn = 0;
    std::set<void *> buffers;
    RpcConn *conn = (RpcConn *)args0;
    ...
    conn->read(&callback_out, sizeof(callback_out));
    CUresult _result;
    if(conn->prepare_response() != RpcError::OK) {
        std::cerr << "Failed to prepare response" << std::endl;
        rtn = 1;
    ...
    _result = cuLogsRegisterCallback(callbackFunc, userData, callback_out);
    conn->write(&_result, sizeof(_result));
    if(conn->submit_response() != RpcError::OK) {
        std::cerr << "Failed to submit response" << std::endl;
        rtn = 1;
...
int handle_cuLogsUnregisterCallback(void *args0) {
#ifdef DEBUG
    std::cout << "Handle function cuLogsUnregisterCallback called" << std::endl;
#endif
    int rtn = 0;
    std::set<void *> buffers;
    RpcConn *conn = (RpcConn *)args0;
    ...
    _result = cuLogsUnregisterCallback(callback);
    conn->write(&_result, sizeof(_result));
    if(conn->submit_response() != RpcError::OK) {
        std::cerr << "Failed to submit response" << std::endl;
        rtn = 1;
...
int handle_cuLogsCurrent(void *args0) {
#ifdef DEBUG
    std::cout << "Handle function cuLogsCurrent called" << std::endl;
#endif
    int rtn = 0;
    std::set<void *> buffers;
    RpcConn *conn = (RpcConn *)args0;
    CUlogIterator *iterator_out;
    conn->read(&iterator_out, sizeof(iterator_out));
    unsigned int flags;
    conn->read(&flags, sizeof(flags));
    CUresult _result;
    if(conn->prepare_response() != RpcError::OK) {
    ...
    _result = cuLogsCurrent(iterator_out, flags);
    conn->write(&_result, sizeof(_result));
    if(conn->submit_response() != RpcError::OK) {
        std::cerr << "Failed to submit response" << std::endl;
        rtn = 1;
...
int handle_cuLogsDumpToFile(void *args0) {
#ifdef DEBUG
    std::cout << "Handle function cuLogsDumpToFile called" << std::endl;
#endif
    int rtn = 0;
    std::set<void *> buffers;
    RpcConn *conn = (RpcConn *)args0;
    CUlogIterator *iterator;
    conn->read(&iterator, sizeof(iterator));
    char *pathToFile = nullptr;
    conn->read(&pathToFile, 0, true);
    unsigned int flags;
    conn->read(&flags, sizeof(flags));
    CUresult _result;
    if(conn->prepare_response() != RpcError::OK) {
    ...
    buffers.insert(pathToFile);
    _result = cuLogsDumpToFile(iterator, pathToFile, flags);
    conn->write(&_result, sizeof(_result));
    if(conn->submit_response() != RpcError::OK) {
        std::cerr << "Failed to submit response" << std::endl;
        rtn = 1;
...
int handle_cuLogsDumpToMemory(void *args0) {
#ifdef DEBUG
    std::cout << "Handle function cuLogsDumpToMemory called" << std::endl;
#endif
    int rtn = 0;
    std::set<void *> buffers;
    RpcConn *conn = (RpcConn *)args0;
    ...
    _result = cuLogsDumpToMemory(iterator, buffer, size, flags);
    if(*size > 0) {
        conn->write(buffer, strlen(buffer) + 1, true);
    }
    conn->write(&_result, sizeof(_result));
    if(conn->submit_response() != RpcError::OK) {
        std::cerr << "Failed to submit response" << std::endl;
